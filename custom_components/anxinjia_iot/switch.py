...
async def async_update_devices(hass: HomeAssistant, access_token: str, eq_numbers: list[str], entities: list[AnxinJiaSwitch]):
    """
    定时更新所有设备的状态。
    ...
    try:
        ...
        all_devices_status = await async_get_all_devices_status(eq_numbers,access_token)
    ...
    for device_info in devices:
        ...
        if device_info.model_type == 102001:
            for virtual_model in device_info.virtual_models:  # 确保使用正确的属性名称
                entity = AnxinJiaSwitch(device_info, virtual_model, access_token)
